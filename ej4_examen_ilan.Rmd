...
Primero leamos los datos con censura

```{r, include=FALSE}
# read data
df_r <- read.csv('datos_ej4.csv',header = F, stringsAsFactors = FALSE, col.names = FALSE) %>% censor_time()
```

### Ejercicio 4.1. Estimar la supervivencia por KM.

```{r, echo=FALSE}
# Kaplan Meyer
table_km <- table_KM(df_r)

# estimates
knitr::kable(table_km)
```

### Ejercicio 4.2. Estiamr $\mu_{85}$
```{r, include=FALSE}
# tao 85
mu85 <- trunc_mean2(tj = table_km$tj,Sj = table_km$St, tao = 85)

```

La vida media residual es de `r mu85`.

### Ejercicio 4.3. IC para $S(t_{0.75})$ al nivel del 95% 

```{r, echo = FALSE}
# puntual estimation
t75 <- quantile_est(St = table_km$St, ee = table_km$std_dev.St, tj = table_km$tj, p = 0.75) 

#confidence intervals tp
t75_ci <- quantile_ci(St = table_km$St, ee = table_km$std_dev.St, tj = table_km$tj, p = 0.75, alpha = 0.05) 

# confidence intervals St
table_ci <- table_confidence(df_r, alpha = 0.05)
kable(table_ci)

```

El cuantíl $t_{0.75}$ es `r t75`.


### Gráfica

```{r, echo = FALSE}
plot_ci(df_ci = table_ci, linear = TRUE, log = FALSE)
abline(h = 0.75, col = 3)
